...
def resolve_coref(user_message, last_bot_message, last_user_message=None):
    ...
    doc = nlp(context)
    if doc.has_extension("coref_resolved"):
        try:
        ...
        print("[Coreferee WARNING] coref_resolved extension not found on doc. Returning original text.")
        resolved = user_message
    return resolved
...
async def analyze(request: AnalyzeRequest):
    strategy = AnalysisStrategy(request.strategy) if request.strategy else None
    return analyze_text(
        request.text,
        request.session_id,
        request.prev_bot_response,
